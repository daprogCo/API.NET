...
download_and_extract_csv() {
    printf "📦 Downloading dataset ZIP from Kaggle...\n"

    if ! kaggle datasets download -d "$KAGGLE_DATASET" -p /tmp > /dev/null; then
        printf "❌ Failed to download dataset ZIP.\n" >&2
        return 1
    fi

    printf "🗃️ Extracting dataset...\n"

    if ! unzip -o /tmp/automobile-dataset.zip -d /tmp > /dev/null; then
    ...
    printf "✅ CSV extracted successfully: %s\n" "$CSV_PATH"
    return 0
}
